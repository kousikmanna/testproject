...
module.exports = {
    ...
    getBankDetail: function (req, res) {
        Bank.bankDetail(req.session.user.id, function (err, bank) {
            if (!err) {
                res.json(bank);
            } else {
    ...
    addBankDetail: function (req, res) {
    	sails.log.debug("userId",req.session.user.id);
    	req.body.user=req.session.user.id;
    	req.body.userId=req.session.user.id;
        Bank.add(req.session.user.id, req.body, function (err, bank) {
            if (!err) {
                res.json(bank);
            } else {
                res.status(err.status).json(err);
