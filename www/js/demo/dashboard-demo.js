...
function sms_transaction_graph() {
    $.ajax({ 
        type: 'GET', 
        url: host+'smsTransaction.json', 
        data: { get_param: 'value' }, 
        dataType: 'json',
        success: function (smsTransactionTrend, status) { 

                var smsData = smsTransactionTrend.legend;

                $("#graph2-title").text(smsTransactionTrend.service_name);

                var inputDate = new Date(smsTransactionTrend.date);
                ...
                if (timeDiff == 0) {
                    ...
                    var dispDate = '<span class="bajaj-textColor-black"><strong>'+smsTransactionTrend.date+': </strong></span>';
                    $("#smsGraphHead").append(dispDate);
                }


                for(var x=0;x<smsData.details.length;x++) {
                    var title = smsData.details[x].type;
                    var count = smsData.details[x].count;
                    var color = smsData.details[x].color;

                    var smsGraphHead = '<span class="small text-faded bajaj-textColor-orange" style="color: '+color+' ">'+title+'-</span><span class="small text-faded " style="color: '+color+' "><strong>'+count+'; </strong></span>';

                    $("#smsGraphHead").append(smsGraphHead);
                }

                var sms_data = smsTransactionTrend.legend.chart_details;
        ...
        success: function (couponTrend, status) { 
                ...
                var couponTrend_data = couponTrend.legend.chart_details;
                 ...
                 Morris.Line({
                      ...
                      xkey: 'date',
                      ykeys: ['closed_coupon', 'inProgress_coupon'],
